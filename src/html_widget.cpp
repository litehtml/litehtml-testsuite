...
long html_widget::draw_measure(int number)
{
	...
	if(page)
	{
		auto view_port = m_sig_get_viewport.emit();
...
int html_widget::action_redraw(void* data)
{
...
int html_widget::action_render(void* data)
{
...
int html_widget::action_update_state(void* data)
{
	auto ctl = (html_widget*) data;
	ctl->m_sig_update_state.emit(ctl->get_state());
	return FALSE;
}
